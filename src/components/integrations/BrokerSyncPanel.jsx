...
import { disconnectZerodha, getZerodhaHoldings, getZerodhaLoginUrl, getZerodhaPositions, getZerodhaStatus, mapZerodhaHoldingToPortfolio } from '@/lib/brokerClient';
  ...
  return (
    <section className="rounded-[32px] border border-white/10 bg-[#0b1624]/90 p-6 shadow-[0_24px_80px_rgba(0,0,0,0.24)]">
      ...
      <div className="mt-5 rounded-[24px] border border-white/8 bg-[#111c2c] p-4 text-sm text-slate-400">
        Backend requirement:
        Run <code className="mx-1 rounded bg-black/20 px-2 py-0.5 text-slate-200">npm run dev:server</code>
        and set the whitelisted Zerodha redirect URL to
        <code className="mx-1 rounded bg-black/20 px-2 py-0.5 text-slate-200">http://localhost:8000/api/zerodha/callback</code>.
      </div>
    </section>
  );
}
